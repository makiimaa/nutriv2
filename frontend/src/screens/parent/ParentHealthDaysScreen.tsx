...
type Nav = NativeStackNavigationProp<RootStackParamList, "ParentHealthDays">;
...
export default function ParentHealthDaysScreen() {
  ...
  const fetchStudentsByIds = useCallback(async (ids: string[]) => {
    ...
    try {
      ...
      console.warn("[ParentHealthDays] fetchStudentsByIds:", e?.message || e);
      return {};
    }
  }, []);
  ...
  const initStudents = useCallback(async () => {
    try {
      ...
      console.warn("[ParentHealthDays] init:", e?.message || e);
      setChildIds([]);
      setChildMap({});
      setCurrentId(undefined);
    }
  ...
  const load = useCallback(async (sid?: string) => {
    ...
    try {
      setLoading(true);
      const r = await axiosClient.get("/health/dates-mine", {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      return humanDate(r.date).toLowerCase().includes(s);
    });
  }, [q, rows]);
  ...
  const Item = ({ item }: { item: Row }) => (
    <Pressable
      onPress={() =>
        navigation.navigate("ParentHealthByDate", {
          date: item.date,
          studentId: currentId,
        })
      }
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        <Text style={styles.title}>Sức khỏe — theo ngày</Text>
        <Pressable
      ...
      <View style={styles.searchWrap}>
        ...
        <TextInput
          value={q}
          onChangeText={setQ}
          placeholder="Tìm ngày (2025-09-08 hoặc 08/09/2025)"
