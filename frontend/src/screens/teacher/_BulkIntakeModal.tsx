      ...
      {rows.map((row, idx) => (
        <View key={idx} style={st.itemCard}>
          ...
          <View style={{ alignItems: "flex-end" }}>
            <GhostBtn title="Xoá món" onPress={() => removeRow(idx)} danger />
          </View>
        </View>
      ))}
    </View>
...
export default function BulkIntakeModal({
  visible,
  onClose,
  onSaved,
  mode,
    ...
    try {
      ...
      const q =
        date && /^\d{4}-\d{2}-\d{2}$/.test(date)
          ? `?from=${date}&to=${date}`
          : "";
      const url = `/menus/school/${schoolId}${q}`;
  ...
  const submit = async () => {
    try {
      if (!classId) return Alert.alert("Lỗi", "Thiếu lớp");
      const body = {
        date:
          date && /^\d{4}-\d{2}-\d{2}$/.test(date)
            ? date
      ...
      if (mode === "create") await axiosClient.post("/intake/bulk", body);
      else await axiosClient.put("/intake/bulk", body);
      onSaved();
      Alert.alert(
        "OK",
        mode === "create"
          ? "Đã thêm cho cả lớp (bỏ qua ai đã có)."
          : "Đã cập nhật cả lớp."
      );
    } catch (e: any) {
          ...
          <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
            ...
            <View style={st.brand}>
              ...
              <Text style={st.brandTitle}>
                {mode === "create"
                  ? "Thêm bản ghi cả lớp"
                  : "Cập nhật bản ghi cả lớp"}
              </Text>
            </View>

            {/* Card: ngày + trường + thực đơn */}
            <View style={st.card}>
              <LabeledInput
              ...
              <LabeledInput
                ...
                placeholder="Ghi chú chung cho cả lớp"
              />
            </View>
            ...
            <View style={st.footerBtns}>
              <PrimaryBtn
                title={
                  mode === "create" ? "Thêm cho cả lớp" : "Cập nhật cả lớp"
                }
