...
export default function IntakeDaysScreen() {
  ...
  const loadDays = async () => {
    try {
      ...
      const r = await axiosClient.get(`/intake/dates${q}`);
      setDays(r.data || []);
    } catch (e: any) {
      Alert.alert(
        "Lỗi",
  ...
  const gotoDate = (d: string) =>
    nav.navigate("IntakeByDate", { date: d, classId: myClass?._id });
        ...
        <View style={st.brand}>
          <View style={st.logoWrap}>
            <LinearGradient
              ...
              style={st.logo}
            >
              <Ionicons name="calendar-outline" size={22} color="#0b3d2e" />
            </LinearGradient>
          </View>
          <Text style={st.brandTitle}>Lịch sử bữa ăn theo ngày</Text>
          <Text style={st.brandSub}>
            {myClass ? `Lớp: ${myClass.name}` : "Đang tải lớp…"}
          </Text>
        </View>

        {/* Bộ lọc: chọn khoảng ngày + nhanh */}
        <View style={st.card}>
