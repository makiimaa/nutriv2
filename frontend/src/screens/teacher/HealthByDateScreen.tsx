...
import SingleHealthModal from "./_SingleHealthModal";
import HealthDetailModal from "./_HealthDetailModal";
...
export default function HealthByDateScreen() {
  ...
  const load = async () => {
    try {
      const r = await axiosClient.get(
        `/health/by-date/${date}?classId=${classId}`
      );
      setRows(r.data || []);
    } catch (e: any) {
      Alert.alert(
